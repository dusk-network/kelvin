...
use kelvin::{
    ...
    Annotation, ByteHash, Children, Combine, Compound, Content, Handle,
    HandleMut, HandleOwned, Method, Sink, Source, ValPath, ValPathMut, ValRef,
    ValRefMut,
};
...
#[derive(Clone)]
pub struct BTree<K, V, H: ByteHash>([Handle<Self, H>; M])
where
    Self: Compound<H>;
    ...
    H: ByteHash,
{
    ...
    pub fn insert(&mut self, k: K, v: V) -> io::Result<Option<V>> {
        for i in 0..M {
            let mut slot = self.0.slot_mut(i);
            match slot.inner_mut()? {
                // if None, it's always the first free item
                HandleMut::None => {
                    slot.replace(HandleOwned::Leaf((k, v)));
                    return Ok(None);
                }
                HandleMut::Leaf((leaf_key, _)) => {
                    if leaf_key == &k {
                        Ok(slot.replace(HandleOwned::Leaf((k, v))));
                    } else {
                        unimplemented!()
                    }
                }
    ...
    // pub fn _insert(&mut self, k: K, v: V) -> io::Result<Option<V>> {
    //     let mut search = BTreeSearch:from(&k);
    //     for i in 0..
    //     for (i, h) in .iter().enumerate() {
    //         if let Some(ann) = h.annotation() {
    //             let handle_key: &Key<K> = (*ann).borrow();
    //             if handle_key.borrow() == k {
